...
import com.lilipay.wechat.utils.WeixinPaySignUtils;
...
public class WechatGateServiceImpl implements WechatGateService {
    ...
    public Response<WechatPayUnifiedOrderGateOutput> unifiedOrder( WechatPayUnifiedOrderGateInput wechatPayUnifiedOrderGateInput ) {
        ...
        WeixinPayUnifiedOrderInputXml inputXml = new WeixinPayUnifiedOrderInputXml();
        inputXml.setAppId(appId);
        inputXml.setMemberNo(memberNo);
        inputXml.setNonceStr(WeixinPaySignUtils.getRandomLess32());
        ...
        String reqXml = JAXBUtils.toString(inputXml);
        inputXml.setSign( WeixinPaySignUtils.sign(reqXml, key, CHARSET));
        String requestXml = JAXBUtils.toString(inputXml);

        logger.info("微信统一下单接口请求: {}", requestXml);
        String result = getHttpTemplate().postForString(getHost(weixinParameter.getWeixinHost()) + "/pay/unifiedorder", null, new StringEntity(requestXml, CHARSET), CHARSET);
        logger.info("微信统一下单接口返回: {}", result);

        WeixinPayUnifiedOrderOutputXml outputXml = JAXBUtils.toObject(result, WeixinPayUnifiedOrderOutputXml.class);
        ...
        if (!WeixinPaySignUtils.verifySign(result, key, CHARSET)) {
        ...
        if (Objects.equals("SUCCESS", outputXml.getResultCode())) {
            if (Objects.equals(outputXml.getTradeType(), "APP")) {
                ...
                appPayRequest.setNonceStr(WeixinPaySignUtils.getRandomLess32());
                ...
                String appPayRequestXml = JAXBUtils.toString(appPayRequest);
                appPayRequest.setSign(WeixinPaySignUtils.sign(appPayRequestXml, key, CHARSET));
            ...
            if (Objects.equals(outputXml.getTradeType(), "JSAPI")) {
                WeixinJsapiResignDomain weixinJsapiResignDomain = new WeixinJsapiResignDomain();
                weixinJsapiResignDomain.setAppid(outputXml.getAppId());
                weixinJsapiResignDomain.setNonceStr(WeixinPaySignUtils.getRandomLess32());
                ...
                appPayRequest.setSign(WeixinPaySignUtils.signJsapi(JAXBUtils.toString(weixinJsapiResignDomain), key, CHARSET));
                unifiedOrderResponse.setPrepayId(outputXml.getPrepayId());
                unifiedOrderResponse.setAppPayRequest(appPayRequest);
            }
    ...
    public Response<WechatPayOrderQueryGateOutput> orderQuery( WechatPayOrderQueryGateInput wechatPayOrderQueryGateInput ) {
        ...
        WeixinPayOrderQueryInputXml inputXml = new WeixinPayOrderQueryInputXml();
        inputXml.setAppid(appId);
        ...
        String reqXml = JAXBUtils.toString(inputXml);
        inputXml.setSign(WeixinPaySignUtils.sign(reqXml, key, CHARSET));
        reqXml = JAXBUtils.toString(inputXml);

        logger.info("微信订单查询接口请求参数：{}", reqXml);
        String result = getHttpTemplate().postForString(getHost(parameter.getWeixinHost()) + "/pay/orderquery", null, new StringEntity(reqXml), CHARSET);
        ...
        WeixinPayOrderQueryOutputXml outputXml = JAXBUtils.toObject(result, WeixinPayOrderQueryOutputXml.class);
        ...
        boolean verify = WeixinPaySignUtils.verifySign(result, key, CHARSET);
        if (!verify) {
    ...
    public Response<WechatPayNoticeGateOutput> acceptNotice( WechatPayNoticeGateInput wechatPayNoticeGateInput ) {
        ...
        boolean verify = WeixinPaySignUtils.verifySign(noticeXml, key, CHARSET);
        if (!verify) {
            logger.error("微信订单查询验签失败！返回参数：{}", noticeXml);
            return ResponseUtils.fail("", "微信订单查询接口验签失败！");
        }
        WeixinPayOrderNoticeXml weixinPayOrderNoticeXml = JAXBUtils.toObject(noticeXml, WeixinPayOrderNoticeXml.class);
    ...
    public Response<WechatCloseOrderGateOutput> closeOrder( WechatCloseOrderGateInput wechatCloseOrderGateInput ) {
        ...
        WeixinCloseOrderInputXml closeOrderInputXml = new WeixinCloseOrderInputXml();
        ...
        closeOrderInputXml.setNonceStr(WeixinPaySignUtils.getRandomLess32());
        closeOrderInputXml.setSign(WeixinPaySignUtils.sign(JAXBUtils.toString(closeOrderInputXml), key, CHARSET));
        String requestStr = JAXBUtils.toString(closeOrderInputXml);
        logger.info("开始调用微信关闭订单接口: {}", requestStr);
        String responseStr = getHttpTemplate().postForString(getHost(weixinPayParameter.getWeixinHost()) + "/pay/closeorder", null, new StringEntity(requestStr), CHARSET);
        logger.info("调用微信关闭订单接口返回: {}", responseStr);
        WeixinCloseOrderOutputXml closeOrderOutputXml = JAXBUtils.toObject(responseStr, WeixinCloseOrderOutputXml.class);
        ...
        boolean verify = WeixinPaySignUtils.verifySign(responseStr, weixinPaykey, CHARSET);
        if (!verify) {
    ...
    public Response<WechatPayRefundGateOutput> refund( WechatPayRefundGateInput wechatPayRefundGateInput ) {
        ...
        WeixinPayRefundInputXml refundInputXml = new WeixinPayRefundInputXml();
        refundInputXml.setAppid(wechatPayRefundGateInput.getAppId());
        refundInputXml.setMch_id(wechatPayRefundGateInput.getMchId());
        refundInputXml.setNonce_str(WeixinPaySignUtils.getRandomLess32());
        ...
        refundInputXml.setSign(WeixinPaySignUtils.sign(requestStr, weixinPayRefundRequest.getApiKey(), CHARSET));
        requestStr = JAXBUtils.toString(refundInputXml);

        logger.info("开始调用微信申请退款: {}", refundInputXml);
        HttpClientTemplate refundHttpClient = getHttpTemplate(weixinPayRefundRequest.getMchId());
        String responseStr = refundHttpClient.postForString(getHost(weixinPayRefundRequest.getWeixinHost()) + "/secapi/pay/refund", null, new StringEntity(requestStr, CHARSET), CHARSET);
        logger.info("调用微信申请退款返回: {}", responseStr);

        WeixinPayRefundOutputXml refundOutputXml = JAXBUtils.toObject(responseStr, WeixinPayRefundOutputXml.class);
        ...
        if (Objects.equals(refundResponse.getReturnCode(), "SUCCESS")) {
            ...
            if (!WeixinPaySignUtils.verifySign(responseStr, weixinPayRefundRequest.getApiKey(), CHARSET)) {
    ...
    public Response<WechatPayRefundQueryGateOutput> refundQuery( WechatPayRefundQueryGateInput wechatPayRefundQueryGateInput ) {
        ...
        WeixinPayRefundQueryInputXml refundQueryInputXml = new WeixinPayRefundQueryInputXml();
        ...
        refundQueryInputXml.setNonce_str(WeixinPaySignUtils.getRandomLess32());
        ...
        String requestStr = JAXBUtils.toString(refundQueryInputXml);
        refundQueryInputXml.setSign(WeixinPaySignUtils.sign(requestStr, wechatPayRefundQueryGateInput.getApiKey(), CHARSET));
        requestStr = JAXBUtils.toString(refundQueryInputXml);

        logger.info("开始查询微信退款结果: {}", refundQueryInputXml);
        String responseStr = getHttpTemplate().postForString(getHost(wechatPayRefundQueryGateInput.getWeixinHost()) + "/pay/refundquery", null, new StringEntity(requestStr), CHARSET);
        logger.info("查询微信退款结果返回: {}", responseStr);

        WeixinPayRefundQueryOutputXml refundQueryOutputXml = JAXBUtils.toObject(responseStr, WeixinPayRefundQueryOutputXml.class);
        ...
        if (Objects.equals(refundQueryResponse.getReturnCode(), "SUCCESS")) {
            ...
            if (!WeixinPaySignUtils.verifySign(responseStr, weixinPayRefundQueryRequest.getApiKey(), CHARSET)) {
    ...
    public Response<WechatPayRefundNoticeGateOutput> acceptRefundNotice( WechatPayRefundNoticeGateInput wechatPayRefundNoticeGateInput ) {
        ...
        boolean verifySignResult = WeixinPaySignUtils.verifySign(noticeXml, apiKey, CHARSET);
        if (!verifySignResult) {
            logger.error("微信退款结果通知返回数据验签失败, 返回数据: {}", noticeXml);
            return ResponseUtils.fail("", "微信退款结果通知返回数据验签失败");
        }
        WeixinPayRefundResultNoticeXml refundResultNoticeXml = JAXBUtils.toObject(noticeXml, WeixinPayRefundResultNoticeXml.class);
        ...
        if (Objects.equals(refundResultNoticeXml.getReturn_code(), "SUCCESS")) {
            ...
            String reqInfoXml = WeixinPaySignUtils.decodeRefundReqInfo(reqInfoStr, apiKey);
            WeixinPayRefundResultReqInfoXml refundResultReqInfoXml = JAXBUtils.toObject(reqInfoXml, WeixinPayRefundResultReqInfoXml.class);
    ...
    public Response<WechatOrderRepayGateOutput> orderRepay( WechatOrderRepayGateInput wechatOrderRepayGateInput ) {
        ...
        if (Objects.equals(wechatOrderRepayGateInput.getTradeType(), "APP")) {
            WeixinAppPayRequest appPayRequest = new WeixinAppPayRequest();
            appPayRequest.setAppId(wechatOrderRepayGateInput.getAppId());
            appPayRequest.setPartnerId(wechatOrderRepayGateInput.getMchId());
            appPayRequest.setNonceStr(WeixinPaySignUtils.getRandomLess32());
            appPayRequest.setTimeStamp(String.valueOf(System.currentTimeMillis() / 1000));
            appPayRequest.setWxPackage("Sign=WXPay");
            appPayRequest.setPrepayId(wechatOrderRepayGateInput.getPrepayId());
            String appPayRequestXml = JAXBUtils.toString(appPayRequest);
            appPayRequest.setSign(WeixinPaySignUtils.sign(appPayRequestXml, wechatOrderRepayGateInput.getKey(), CHARSET));
            WeixinOrderRepayResponse weixinOrderRepayResponse = new WeixinOrderRepayResponse();
            weixinOrderRepayResponse.setAppPayRequest(appPayRequest);
            return ResponseUtils.success(weixinOrderRepayResponse);
        }

        if (Objects.equals(wechatOrderRepayGateInput.getTradeType(), "JSAPI")) {
            WeixinJsapiResignDomain weixinJsapiResignDomain = new WeixinJsapiResignDomain();
            weixinJsapiResignDomain.setAppid(wechatOrderRepayGateInput.getAppId());
            weixinJsapiResignDomain.setNonceStr(WeixinPaySignUtils.getRandomLess32());
            weixinJsapiResignDomain.setTimeStamp(String.valueOf(System.currentTimeMillis() / 1000));
            weixinJsapiResignDomain.setWxPackage("prepay_id=" + wechatOrderRepayGateInput.getPrepayId());
            weixinJsapiResignDomain.setSignType("MD5");
            ...
            appPayRequest.setWxPackage(weixinJsapiResignDomain.getWxPackage());
            appPayRequest.setSignType(weixinJsapiResignDomain.getSignType());
            appPayRequest.setSign(WeixinPaySignUtils.signJsapi(JAXBUtils.toString(weixinJsapiResignDomain), wechatOrderRepayGateInput.getKey(), CHARSET));
            WeixinOrderRepayResponse weixinOrderRepayResponse = new WeixinOrderRepayResponse();
            weixinOrderRepayResponse.setAppPayRequest(appPayRequest);
            return ResponseUtils.success(weixinOrderRepayResponse);
        }
        return ResponseUtils.fail("", "暂不支持重签");
    }

    @Override
    public Response<WechatTransfersGateOutput> transfers( WechatTransfersGateInput wechatTransfersGateInput ) {

        WeixinTransfersInput input = new WeixinTransfersInput();
        ...
        input.setNonce_str(WeixinPaySignUtils.getRandomLess32());
        ...
        input.setSign(WeixinPaySignUtils.sign(JAXBUtils.toString(input), wechatTransfersGateInput.getApiKey(), CHARSET));
        String xml = JAXBUtils.toString(input);
        HttpClientTemplate refundHttpClient = getHttpTemplate(wechatTransfersGateInput.getMchid());
        String outputXml = refundHttpClient.postForString(getHost(wechatTransfersGateInput.getWeixinHost()) + "/mmpaymkttransfers/promotion/transfers", null, new StringEntity(xml, CHARSET), CHARSET);
        WeixinTransfersOutput output = JAXBUtils.toObject(outputXml, WeixinTransfersOutput.class);
        WeixinTransfersResponse response = WeixinTransfersResponseBuilder.aWeixinTransfersResponse().withWeixinTransfersOutput(output).build();
        return ResponseUtils.success(response);
    }
    ...
    public Response<WechatTransfersQueryGateOutput> transfersQuery( WechatTransfersQueryGateInput wechatTransfersQueryGateInput ) {
        ...
        WeixinTransfersQueryInput input = new WeixinTransfersQueryInput();
        input.setNonce_str(WeixinPaySignUtils.getRandomLess32());
        ...
        input.setSign(WeixinPaySignUtils.sign(JAXBUtils.toString(input), wechatTransfersQueryGateInput.getApiKey(), CHARSET));
        String xml = JAXBUtils.toString(input);
        HttpClientTemplate refundHttpClient = getHttpTemplate(wechatTransfersQueryGateInput.getMchid());
        String outputXml = refundHttpClient.postForString(getHost(wechatTransfersQueryGateInput.getWeixinHost()) + "/mmpaymkttransfers/gettransferinfo", null, new StringEntity(xml, CHARSET), CHARSET);
        WeixinTransfersQueryOutput output = JAXBUtils.toObject(outputXml, WeixinTransfersQueryOutput.class);
        return ResponseUtils.success(WeixinTransfersQueryResponseBuilder.aWeixinTransfersQueryResponse().withWeixinTransfersQueryOutput(output).build());
    }
}
